...
plot.r.phi.figures=function(nameIn, t, tw, wmin, wmax, ub, uk, vb, vk, lm, lk, ld, alpha, omega, phi, mb, mx, my, r, cost, beta, h, plot_legend = FALSE){
  x.seq = seq(0.05,1.00,length=4)
  y.seq = seq(0.02,0.18,length=5)
  name = paste(nameIn,"_r-phi","_B",beta,"_uk",10*uk,"_vk",10*vk,"_lm",10*lm,"_mX",10*mx,"_mY",10*my,"_vb",vb*10,"_r","[x]","_c",cost*100,"_phi","[y]",sep = "")
...
plot.vb.my.figures=function(nameIn, t, tw, wmin, wmax, ub, uk, vb, vk, lm, lk, ld, alpha, omega, phi, mb, mx, my, r, cost, beta, h, plot_legend = FALSE){
  x.seq = seq(1.0,2.0,length=5)
  y.seq = seq(0.0,2.0,length=5)
  name = paste(nameIn,"_vb-my","_B",beta,"_uk",10*uk,"_vK",10*vk,"_lm",10*lm,"_mX",10*mx,"_mY","[y]","_vb","[x]","_r",r,"_c",cost*100,"_phi",phi*100,sep = "")
...
#light effects
lm = 1.0		#REQUIRE non-zero value! predation efficiency at noon
lk = 5.0		#determines the sensitivity for small light
...
plot.vb.my.figures("fig5", t, tw, wmin, wmax, ub, uk, vb, vk, lm, lk, ld, alpha, omega, phi, mb, mx, my, r, cost, beta, h,plot_legend = TRUE)
	
# FIGURE A3 simple models
for(lm in c(1.0,0.2)){# effect of light
  for(mx in c(0.0,1.0)){ # other predators
    for(beta in c(0.0,1.0)){ # effect of speed
    for(uk in c(0.0,0.2)){# effect of temp on speed
      vk=uk
      plot.vb.my.figures("figA3", t, tw, wmin, wmax, ub, uk, vb, vk, lm, lk, ld, alpha, omega, phi, mb, mx, my, r, cost, beta, h,plot_legend = TRUE)
      }
    }
  }
}
...
beta=1.0
# FIGURE 6 A4 A5 
for(lm in c(0.1,0.5,1.0)){
  for(mx in c(0.0,0.5,1.0,2.0)){
    for(my in c(0.0,0.5,1.0)){
      #=== draw r-phi figures === 
    	plot.r.phi.figures("fig6",t, tw, wmin, wmax, ub, uk, vb, vk, lm, lk, ld, alpha, omega, phi, mb, mx, my, r, cost, beta, h,plot_legend = TRUE)
    }
  }
}
